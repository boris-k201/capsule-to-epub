...
def process_posts_list(body: str):
    print(body)
...
print(
    f'Date range to parse: {initial_date.strftime("%Y-%m-%d")} to {final_date.strftime("%Y-%m-%d")} UTC\n'
)
...
include_all_urls_input = input(
    f"Include only allowed URLs (from allowed_urls.txt)? (Enter for Yes): "
)
